...
void P_SetPsprite (player_t *player, int position, FState *state)
{
	...
	do
	{
		...
		{
			size_t index=state->GetMisc1_2();
			if (index>0 && index<StateParameters.Size()-2)
			{
				if (StateParameters[index])
				{ // Set coordinates.
					psp->sx = StateParameters[index]<<FRACBITS;
				}
				if (StateParameters[index+1])
				{
					psp->sy = StateParameters[index+1]<<FRACBITS;
				}
			}
		}
		if (state->GetAction())
...
void A_ReFire (AActor *actor)
{
	...
	if ((player->cmd.ucmd.buttons&BT_ATTACK) && !player->ReadyWeapon->bAltFire 
		&& player->PendingWeapon == WP_NOCHANGE && player->health)
	{
		player->refire++;
		P_FireWeapon (player);
	}
	else if ((player->cmd.ucmd.buttons&BT_ALTATTACK) && player->ReadyWeapon->bAltFire
		&& player->PendingWeapon == WP_NOCHANGE && player->health)
	{
		player->refire++;
		P_FireWeaponAlt (player);
