...
//
//==========================================================================
const TypeInfo * T_GetMobjType(svalue_t arg)
{
	const TypeInfo * typeinfo=NULL;
	
	if (arg.type==svt_string)
	{
		typeinfo=TypeInfo::FindType(arg.value.s);

		// invalid object to spawn
		if(!typeinfo) script_error("unknown object type: %s\n", arg.value.s); 
	}
	else if (arg.type==svt_mobj)
	{
		AActor * mo = MobjForSvalue(arg);
		if (mo) typeinfo = mo->GetClass();
	}
	else
	{
		int objtype = intvalue(arg);
		if (objtype>=0 && objtype<=139) typeinfo=TypeInfo::FindType(ActorNames[objtype]);
		else typeinfo=NULL;

		// invalid object to spawn
		if(!typeinfo) script_error("unknown object type: %i\n", objtype); 
	}
	return typeinfo;
}
...
//
//==========================================================================
static const TypeInfo * T_GetAmmo(svalue_t t)
{
	const char * p;
	...
	const TypeInfo * am=TypeInfo::FindType(p);
...
/*        static char inputstr[128];
  ...
  gets(inputstr);
	...
	*/
	Printf(PRINT_BOLD,"input() function not available in doom\a\n");
}
...
void SF_Tip(void)
{
	...
	if(!current_script->trigger || !current_script->trigger->player) return;
	
	// haleyjd: made consistent with other tip functions
	for(i=0; i<t_argc; i++)
		sprintf(tempstr,"%s%s", tempstr, stringvalue(t_argv[i]));
    
	if (current_script->trigger->CheckLocalView(current_script->trigger->player-players)) C_MidPrint(tempstr);
}
...
void SF_PlayerTip(void)
{
	...
	if (players[plnum].mo->CheckLocalView(plnum)) C_MidPrint(tempstr);
}
...
void SF_Spawn(void)
{
	...
	if (!(typeinfo=T_GetMobjType(t_argv[0]))) return;
	...
	if(t_argc >= 5)
	{
		...
		z = (GetDefaultByType(typeinfo)->flags & MF_SPAWNCEILING) ? ONCEILINGZ : ONFLOORZ;
	...
	t_return.type = svt_mobj;
	t_return.value.mobj = Spawn(typeinfo, x, y, z);

	// check whether it fits or not - this was missing and caused problems!
	if (!t_return.value.mobj)
	{
		t_return.value.mobj = NULL;
	}
	else if (!P_TestMobjLocation(t_return.value.mobj))
	{
...
void SF_KillObj(void)
{
	...
	// [GrafZahl] This called P_KillMobj directly which is a very
	// bad thing to do!
	if(mo) P_DamageMobj(mo, NULL, NULL, mo->health, MOD_UNKNOWN);
}
...
{
	int tag;    // dummy line for teleport function
	AActor *mo;
...
void SF_DamageObj(void)
{
	...
	if(mo)
		P_DamageMobj(mo, NULL, current_script->trigger, damageamount, MOD_UNKNOWN);
}
...
void SF_PushThing(void)
{
	...
	angle = (unsigned int)FixedToAngle(fixedvalue(t_argv[1]));
...
void SF_ReactionTime(void)
{
	...
	{
		if(mo) 
			mo->reactiontime = (intvalue(t_argv[1]) * TICRATE) / 100;
	}
...
void SF_SetCamera(void)
{
	...
	angle = t_argc < 2 ? mo->angle : (fixed_t)FixedToAngle(fixedvalue(t_argv[1]));
	...
	if (newcamera)
	{
		...
		newcamera->z = t_argc < 3 ? (mo->z + (41 << FRACBITS)) : (intvalue(t_argv[2]) << FRACBITS);
...
void SF_SetLineTexture(void)
{
	...
	{ 
		...
		{ 
			if(lines[i].sidenum[side]!=NO_INDEX)
			{ 
...
void SF_MaxPlayerAmmo()
{
	...
	else if(t_argc >= 3)
	{
		...
		if (!iammo) 
		{
			iammo = static_cast<AAmmo *>(Spawn (ammotype, 0, 0, 0));
			iammo->Amount = 0;
...
void SF_PlayerWeapon()
{
	...
	const TypeInfo * ti = TypeInfo::FindType(WeaponNames[weaponnum]);
	if (!ti)
	{
...
void SF_PlayerSelectedWeapon()
{
	...
	for(int i=0;i<9;i++)
	{
		if (!strcmp(WeaponNames[i], players[playernum].ReadyWeapon->GetClass ()->Name+1))
		{
			t_return.value.i=i;
			break;
		}
...
void SF_GiveInventory(void)
{
	...
	if(t_argc < 2)
	{
		script_error("insufficient arguments to giveinventory\n");
		return;
	}
...
void SF_TakeInventory(void)
{
	...
	if(t_argc < 2)
	{
		script_error("insufficient arguments to takeinventory\n");
		return;
	}
...
void SF_CheckInventory(void)
{
	...
	if(t_argc < 2)
	{
		script_error("insufficient arguments to checkinventory\n");
		return;
	}
...
void SF_SetWeapon()
{
	if(t_argc < 2)
	{
		script_error("insufficient arguments to setweapon\n");
		return;
	}
	int playernum=T_GetPlayerNum(t_argv[0]);
	if (playernum!=-1) 
	{
		AInventory *item = players[playernum].mo->FindInventory (TypeInfo::FindType (stringvalue(t_argv[1])));
...
void SF_SpawnExplosion()
{
	...
	if (spawn)
	{
		...
		t_return.value.i = spawn->SetState(spawn->DeathState);
...
void SF_RadiusAttack()
{
    ...
    if (spot && source)
    {
        P_RadiusAttack(spot, source, damage, damage, MOD_UNKNOWN, true);
    }
}
...
void SF_SpawnMissile()
{
	...
	if (!(typeinfo=T_GetMobjType(t_argv[2]))) return;

    mobj = MobjForSvalue(t_argv[0]);
    target = MobjForSvalue(t_argv[1]);
	if (mobj && target) P_SpawnMissile(mobj, target, typeinfo);
}
...
{					//DarkWolf95:December 7, 2003: Change to set only
	...
	{
		case 1:
			newstate = mo->SpawnState;
			break;
		case 2:
			newstate = mo->SeeState;
			break;
		case 3:
			newstate = mo->MissileState;
			break;
		case 4:
			newstate = mo->MeleeState;
			break;
		case 5:
			newstate = mo->PainState;
			break;
		case 6:
			newstate = mo->DeathState;
			break;
		case 7:
			newstate = mo->RaiseState;
			break;
		case 8:
			newstate = mo->XDeathState;
			break;
		case 9:
			newstate = mo->CrashState;
			break;
		default:
		...
		t_return.type = svt_int;
		t_return.value.i = mo->SetState(newstate);
}
...
void SF_Resurrect()
{
	...
	if(!mo->RaiseState)  //Don't resurrect things that can't be resurrected
		return;

	mo->SetState(mo->RaiseState);
...
void SF_LineAttack()
{
	...
	P_LineAttack(mo, angle, MISSILERANGE, slope, damage, MOD_UNKNOWN, RUNTIME_CLASS(ABulletPuff));
}
...
void SF_ObjType()
{
	...
	for(int i=0;i<=139;i++) if (!strcmp(ActorNames[i], mo->GetClass ()->Name+1))
	{
		t_return.type = svt_int;
		t_return.value.i = i;
		return;
...
void SF_ThingCount(void)
{
	...
	if (t_argc<2 || intvalue(t_argv[1])==0)
	{
		while (mo=it.Next()) 
		{
			if (mo->IsA(typeinfo))
	...
	else
	{
		while (mo=it.Next())
		{
			if (mo->IsA(typeinfo) && mo->health>0) count++;
		}
	}
	t_return.type = svt_int;
...
void SF_SpawnShot2(void)
{
	...
	AActor *mo = Spawn (typeinfo, source->x, source->y, source->z+z);
	if (mo) 
	{
...
void  SF_KillInSector()
{
	...
	while (mo=it.Next())
	{
		if (mo->flags3&MF3_ISMONSTER && mo->Sector->tag==tag) P_DamageMobj(mo, NULL, NULL, 1000000, MOD_UNKNOWN);
	}
}
