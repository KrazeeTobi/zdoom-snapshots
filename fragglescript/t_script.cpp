...
void T_FixScript(script_t * script,AActor * notold,AActor * old)
{
	if (script->trigger==old) script->trigger=notold;
}

// FraggleScript sucks so badly...
void T_FixPointers(DObject * notold ,DObject * old)
{
	// Don't bother with this shit when no FS present.
	if (HasScripts)
	{
		if (trigger_obj==old) trigger_obj=(AActor*)notold;
