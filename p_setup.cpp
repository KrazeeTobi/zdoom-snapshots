		...
		) == -1)
	{
		...
		else	// Support for Legacy's color format!
		{
			...
			if (l>=7) 
			{
				...
				int factor = l==7? 25 : clamp<int> ((name2[6]&223)-'A', 0, 25);
				...
				red   = clamp<int>(red  *factor/25 + 255*(25-factor)/25, 0, 255);
				green = clamp<int>(green*factor/25 + 255*(25-factor)/25, 0, 255);
...
void P_LoadThings (int lump, int position)
{
	...
	for ( ; mt < lastmt; mt++)
	{
		...
		else
		{
			...
			if (flags & 0x200		)		mt2.flags |= MTF_DORMANT;
		}
...
void P_VavoomSlope(sector_t * sec, int id, fixed_t x, fixed_t y, fixed_t z, int which)
{
	for (int i=0;i<sec->linecount;i++)
	{
		...
		// Yes, that's a crappy algorithm but what do you expect from Vavoom???
		if (l->args[0]==id)
		{
			vec3_t v1, v2, cross;
			...
			//plane->ic = FLOAT2FIXED (1.f/cross[2]);
			srcplane->ic = FixedDiv/*DivScale32*/ (FRACUNIT, srcplane->c);
...
void P_SetupLevel (char *lumpname, int position)
{
	...
	if (!ForceNodeBuild)
	{
		...
		if (Wads.LumpLength (lumpnum + ML_GLZNODES) != 0)
		{
			// If normal nodes are not present but GL nodes are, use them.
			// (I reversed the order of checking here.
			// GL nodes should be preferred at all times.)
			test = Wads.OpenLumpNum (lumpnum + ML_GLZNODES);
			idcheck = MAKE_ID('Z','G','L','N');
		}
		else //if (Wads.LumpLength (lumpnum + ML_ZNODES) != 0)
		{
	...
	if (ForceNodeBuild)
	{
		...
		FNodeBuilder builder (leveldata, polyspots, anchors, true/*genglnodes*/);
...
void P_Init ()
{
	...
	P_InitTerrainTypes ();
	R_InitSprites ();
	P_InitKeyMessages();	// Must be done here so the cheating code knows about all valid keys!
}
