		...
		if (line->sidenum[side] != NO_INDEX)
		{
			...
			if (base != NULL)
			{
				...
				if (den != 0)
				{
					...
					if (line->sidenum[side^1]!=NO_INDEX && sides[line->sidenum[side^1]].sector->e->ffloors.Size())
					{
...
bool AActor::FloorBounceMissile (secplane_t &plane)
{
	...
	// FIXME: This desperately needs a better implementation.
	if (flags4&MF4_STRIFEDAMAGE)
	{
		// The reflected velocity keeps only about 50% of its original speed for Strife's grenades
	...
	// Strife's grenades don't make a bouncing sound
	if (SeeSound && !(flags&MF4_STRIFEDAMAGE))
	{
				...
				(gameinfo.gametype != GAME_Doom || !(mo->flags & MF_NOCLIP)))
			{
				...
				else
				{
						...
						!(mo->flags3 & MF3_SKYEXPLODE))
					{
						...
						// [CO] not on 3D-floors above a sky floor!
						if (!mo->Sector->e->ffloors.Size() || mo->floorz==mo->Sector->floorplane.ZatPoint(mo->x,mo->y)) 
						{
							mo->Destroy ();
							return;
						}
...
// [RH] Need to recheck this because the sector action might have
		...
		if (mo->z + mo->height > mo->ceilingz)
		{
				...
				(gameinfo.gametype != GAME_Doom || !(mo->flags & MF_NOCLIP)))
			{
				...
				{
					// [CO] not under 3D-floors below a sky!
					if (!mo->Sector->e->ffloors.Size() || mo->floorz==mo->Sector->ceilingplane.ZatPoint(mo->x,mo->y)) 
					{
						mo->Destroy ();
						return;
					}
			...
			if (player != NULL)
			{
				...
				{ // Hexen scroll special
					...
					if (compatflags&COMPATF_RAVENSCROLL)
					{
						...
						fixed_t carryspeed = FixedDiv(HexenSpeedMuls[scrolltype % 3] * 2048, CARRYFACTOR);
					...
					else
					{
						...
						scrollx +=FixedDiv(-3*1024*hexenScrollies[scrolltype][0], CARRYFACTOR);
						scrolly +=FixedDiv( 3*1024*hexenScrollies[scrolltype][1], CARRYFACTOR);
					}
				}
				...
				{ // Heretic scroll special
					...
					{
						// Use speeds that actually match the scrolling textures!
						carryspeed = 3072 * (1<<(scrolltype%5));
					}
					carryspeed = FixedDiv(carryspeed, CARRYFACTOR);
				...
				{ // Strife scroll special
					...
					fixed_t carryspeed = FixedDiv((anglespeed % 10) << (FRACBITS - 4), CARRYFACTOR);
...
void P_BloodSplatter (fixed_t x, fixed_t y, fixed_t z, AActor *originator)
{
	...
	if (cl_bloodtype >= 1)
	{
		P_DrawSplash2 (40, x, y, z, R_PointToAngle2 (originator->x , x, originator->y , y), 2, bloodcolor);
	}
}
...
{
	fixed_t x, y, z;
	PalEntry bloodcolor = (PalEntry)mo->GetClass()->Meta.GetMetaInt(AMETA_BloodColor);
