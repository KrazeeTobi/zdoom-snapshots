...
void A_MonsterRail (AActor *actor)
{
	...
	P_RailAttack (actor, actor->damage, 0,
		ColorMatcher.Pick (0, 0, 255), ColorMatcher.Pick (255, 255, 255));
}
...
//
//---------------------------------------------------------------------------
CVAR(Int, dropstyle, 0, CVAR_ARCHIVE);
...
void A_BossDeath (AActor *actor)
{
	...
	while (sa)
	{
		if (!strcmp(sa->type,actor->GetClass()->Name+1))
		{
			if (!checked && !CheckBossDeath(actor)) return;
			checked=true;
