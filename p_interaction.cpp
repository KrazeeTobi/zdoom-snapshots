...
void P_DamageMobj (AActor *target, AActor *inflictor, AActor *source, int damage, int mod, int flags)
{
	...
	// Cheap ass MoD exclusion
	if ((!target->DeathState) && (target->IDeathState || target->BDeathState || target->EDeathState))
	{
		if (mod==MOD_ICE)
		{
			if (!target->IDeathState) return;
		}
		else if (mod==MOD_FIRE)
		{
			if (!target->BDeathState) return;
		}
		else if (mod==MOD_DISINTEGRATE)
		{
			if (!target->EDeathState) return;
		}
		else return;
	}
	...
	// [GrafZahl] A missing pain state should not make the monster disappear!
	if ((pr_damagemobj() < target->PainChance && target->PainState)
