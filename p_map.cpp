...
CVAR (Bool, sv_smartaim, false, CVAR_ARCHIVE|CVAR_SERVERINFO)
...
int P_GetFriction (const AActor *mo, int *frictionfactor)
{
	...
	{	// When the object is straddling sectors with the same
		...
		for (m = mo->touching_sectorlist; m; m = m->m_tnext)
		{
			...
			for(size_t i=0;i<sec->e->ffloors.Size();i++)
			{
...
/*		Printf ("%d:%d: %d  (%d %d %d %d)  (%d %d %d %d)\n", level.time, ld-lines, r,
	...
	if (rail &&
		...
		gameinfo.gametype != GAME_Strife || level.flags & LEVEL_HEXENFORMAT))
	{
...
//				abs(thing->y - tmy) <= thing->radius)
	...
	if (tmthing->flags & MF_MISSILE)
	{
		...
		if (damage)
		{
			...
			// No blood decal when there is a blood splatter sprite?
...
// Any contacted lines the step closer together will adjust them.
	...
	if(tmsector->e->ffloors.Size())
	{
		...
		for(size_t i=0;i<tmsector->e->ffloors.Size();i++)
		{
			...
			if(ff_bottom < tmceilingz && abs(delta1) >= abs(delta2)) tmceilingz = ff_bottom;
		}
	}
				...
				bool onfloor) // [RH] Let P_TryMove keep the thing on the floor
{
	...
	if (!(thing->flags & MF_NOCLIP))
    {
		...
		if (!dropoff && !(thing->flags & (MF_DROPOFF|MF_FLOAT|MF_MISSILE)))
		{
			...
			if (!(thing->flags4&MF4_AVOIDINGDROPOFF))
			{
			...
			else
			{
				...
				if (thing->dropoffz - tmdropoffz > gameinfo.StepHeight) return false;
			}
		}
...
bool P_CheckSlopeWalk (AActor *actor, fixed_t &xmove, fixed_t &ymove)
{
	...
	for(unsigned int i=0;i<actor->floorsector->e->ffloors.Size();i++)
	{
		...
		if (thisplanez>planezhere && thisplanez<=actor->z+gameinfo.StepHeight)
		{
			plane = rover->top.plane;
...
bool P_AimTraverse3DFloors(intercept_t * in)
{
    ...
    if(li->frontsector->e->ffloors.Size() || li->backsector->e->ffloors.Size())
    {
		...
		for(int i=1;i<=2;i++)
		{
			...
			for(size_t k=0;k<s->e->ffloors.Size();k++)
			{
...
fixed_t P_AimLineAttack (AActor *t1, angle_t angle, fixed_t distance, fixed_t vrange)
{
	...
	for(size_t i=0;i<t1->Sector->e->ffloors.Size();i++)
	{
					   ...
					   int pitch, int damage, int damageType, const TypeInfo *pufftype)
{
	...
	else
	{
		...
		else
		{
			...
			// AXEBLOOD is a weapon flag so if it set it should also work in Doom, right?
...
BOOL PTR_UseTraverse (intercept_t *in)
{
	...
	if (!in->isaline)
	{
		...
		// enable activation of puzzle items by pressing +use
		if (in->d.thing->special==UsePuzzleItem)
		{
			if (LineSpecials[UsePuzzleItem](NULL, usething, false, in->d.thing->args[0], in->d.thing->args[1], 0, 0, 0))
				return false;
		}
