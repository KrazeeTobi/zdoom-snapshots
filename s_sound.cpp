...
static char LastLocalSndInfo[9]="";
static char LastLocalSndSeq[9]="";
...
void S_Init ()
{
	...
	*LastLocalSndInfo = *LastLocalSndSeq=0;
...
void S_Start ()
{
	...
	if (GSnd)
	{
		...
		char * LocalSndInfo= level.info? level.info->soundinfo : "";
		char * LocalSndSeq = level.info? level.info->sndseq : "";
		...
		else if (stricmp(LastLocalSndSeq, LocalSndSeq))
		{
			S_ParseSndSeq(Wads.CheckNumForName(LocalSndSeq));
		}
		strcpy(LastLocalSndInfo, LocalSndInfo);
		strcpy(LastLocalSndSeq , LocalSndSeq);
	}
...
// Sure, I could pass a FileReader to I_RegisterSong but internally 
// it'd also read the lump into memory and this is easier to handle.
