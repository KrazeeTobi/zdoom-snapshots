...
void FWadCollection::AddFile (const char *filename, const char * data, int length)
{
	...
	else if (header.magic==ZIP_ID)
	{
		...
		NumLumps += SHORT(info.wEntryCount);
		LumpInfo = (LumpRecord *)Realloc (LumpInfo, NumLumps*sizeof(LumpRecord));
		lump_p = &LumpInfo[startlump];

		// Load the entire central directory. Too bad that this contains variable length entries...
		directory = malloc(LONG(info.dwCDSize));
		wadinfo->Seek(LONG(info.dwCDOffset), SEEK_SET);
		wadinfo->Read(directory, LONG(info.dwCDSize));

		char * dirptr =(char*)directory;
		for (int i = 0; i < SHORT(info.wEntryCount); i++)
		{
			...
			int len = SHORT(zip_fh->wFileNameSize);
			strncpy(name, dirptr + sizeof(FZipFileInfo), MIN<int>(len, 255));
			name[len]=0;
			dirptr += sizeof(FZipFileInfo) + 
					  SHORT(zip_fh->wFileNameSize) + 
					  SHORT(zip_fh->wExtraSize) + 
					  SHORT(zip_fh->wCommentSize);
			
			// skip Directories
			if(name[len - 1] == '/' && LONG(zip_fh->dwSize) == 0) 
			{
				skipped++;
				continue;
			}
			...
			if(SHORT(zip_fh->wCompression) != 0 && SHORT(zip_fh->wCompression) != Z_DEFLATED)
			{
			...
			if(SHORT(zip_fh->wFlags) & ZF_ENCRYPTED)
			{
			...
			lump_p->position = LONG(zip_fh->dwFileOffset) + sizeof(FZipLocalHeader) + SHORT(zip_fh->wFileNameSize);
			...
			lump_p->wadnum = (WORD)NumWads;
			lump_p->flags = SHORT(zip_fh->wCompression) == Z_DEFLATED? LUMPF_COMPRESSED : 0;
			lump_p->compressedsize = LONG(zip_fh->dwCompressedSize);
			...
			wadinfo->Seek(LONG(zip_fh->dwFileOffset), SEEK_SET);
			wadinfo->Read(&localHeader, sizeof(localHeader));
			lump_p->position += SHORT(localHeader.wExtraSize);
			lump_p++;
		}
	...
	if (EmbeddedWADs.Size())
	{
		...
		for(int i=0; i < EmbeddedWADs.Size(); i++)
		{
			...
			*wadstr=0;
			...
			DWORD size = LONG(zip_fh->dwSize);
			char * buffer = new char[size];

			int position = LONG(zip_fh->dwFileOffset) ;

			wadinfo->Seek(position, SEEK_SET);
			wadinfo->Read(&localHeader, sizeof(localHeader));
			position += SHORT(localHeader.wExtraSize) + sizeof(FZipLocalHeader) + SHORT(zip_fh->wFileNameSize);
